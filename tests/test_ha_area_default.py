...
class TestIsBareLightPhrase:
    ...
    def test_bare_light_phrases(self, phrase):
        ...
        assert devices._is_bare_light_phrase(phrase) is True
    ...
    def test_not_bare_light_phrases(self, phrase):
        ...
        assert devices._is_bare_light_phrase(phrase) is False
...
class TestCurrentSatelliteHaArea:
    def test_no_live_assistant_returns_none(self):
        ...
        assert devices._current_satellite_ha_area() is None
    ...
    def test_returns_the_calling_satellites_area(self):
        ...
        try:
            assert devices._current_satellite_ha_area() == "kitchen"
        finally:
...
class TestBareLightAreaEntities:
    def test_resolves_area_lights_excluding_denied(self, satellite_with_area):
        ...
        with (
            patch.object(client, "_AREA_MAP", {"kitchen": "Kitchen"}),
            patch.object(client, "_DENIED_ENTITIES", {"light.kitchen_lamp"}),
            patch.object(client, "_render_template", return_value=json.dumps(entities)),
        ):
            result = devices._bare_light_area_entities("lights")
    ...
    def test_filler_words_are_stripped_before_matching(self, satellite_with_area):
        ...
        with (
            ...
            result = devices._bare_light_area_entities("the lights")
    ...
    def test_explicit_room_bypasses_the_fallback(self, satellite_with_area):
        ...
        assert devices._bare_light_area_entities("office lights") is None

    def test_all_qualifier_bypasses_the_fallback(self, satellite_with_area):

        assert devices._bare_light_area_entities("all the lights") is None

    def test_no_satellite_area_configured_returns_none(self):
        from core.satellite_context import set_current_assistant

        set_current_assistant(None)
        assert devices._bare_light_area_entities("lights") is None

    def test_area_with_no_lights_returns_none(self, satellite_with_area):

        with (
            patch.object(client, "_AREA_MAP", {"kitchen": "Kitchen"}),
            patch.object(client, "_render_template", return_value=json.dumps(["switch.kettle"])),
        ):
            assert devices._bare_light_area_entities("lights") is None
    ...
    def test_ha_unreachable_returns_none(self, satellite_with_area):
        ...
        with (
            patch.object(client, "_AREA_MAP", {"kitchen": "Kitchen"}),
            patch.object(client, "_render_template", return_value=None),
        ):
            assert devices._bare_light_area_entities("lights") is None
...
class TestToolsUseAreaDefault:
    def test_turn_off_targets_every_light_in_the_area(self, satellite_with_area):
        ...
        with (
            ...
            result = devices.turn_off("lights")
    ...
    def test_turn_off_explicit_room_uses_normal_resolution(self, satellite_with_area):
        ...
        with (
            patch.object(client, "HA_TOKEN", "tok"),
            patch.object(client, "_loaded", True),
            patch.object(client, "_ENTITY_ALIASES", {"office lights": "light.office"}),
            patch("tools.ha_client.requests.post") as post,
        ):
            post.return_value = MagicMock(status_code=200)
            devices.turn_off("office lights")
    ...
    def test_set_brightness_applies_to_every_light_in_the_area(self, satellite_with_area):
        ...
        with (
            ...
            post.return_value = MagicMock(status_code=200)
            result = devices.set_ha_brightness("lights", 40)
