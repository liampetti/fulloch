...
GROUPS = (
    "General", "Voice", "Endpointing", "Dashboard",
    "Home Assistant", "Notes", "Search",
)
...
SCHEMA: tuple = (
    ...
    Field("general", "tts_speed", "float", "Voice", 1.0,
          ...
          "reference recording.", apply=HOT),
    Field("general", "barge_in", "enum", "Voice", "wakeword",
          "'off' (half-duplex) or 'wakeword' (interrupt mid-response).",
          choices=("off", "wakeword"), apply=HOT),
    Field("general", "follow_up_time", "str", "Voice", "5s",
          "Wakeword-free reply window after TTS ('0s' to disable), for "
          "back-and-forth in quiet rooms.", apply=HOT),

    # --- Endpointing -------------------------------------------------------
    Field("general", "use_vad", "bool", "Endpointing", True,
          "Endpoint on Silero speech probability rather than mic energy — "
          "robust in noisy rooms; drops coughs/taps before ASR.", apply=HOT),
    Field("general", "barge_in_threshold_dbfs", "float", "Endpointing", -48,
          "Silence floor while speaking, in dBFS. Lower improves barge-in; "
          "raise if the assistant interrupts itself.", apply=HOT),
    Field("general", "vad_threshold", "float", "Endpointing", 0.6,
          "Probability (0..1) above which a window counts as voice. Raise "
          "toward 0.7 in noisy rooms; lower for quiet/soft speech.", apply=HOT),
    Field("general", "vad_endpoint_silence_ms", "int", "Endpointing", 1500,
          "Low-probability duration before end-of-speech is declared.", apply=HOT),
    Field("general", "vad_min_speech_ms", "int", "Endpointing", 300,
          "Min voiced duration sent to ASR (outside the follow-up window); "
          "filters coughs/taps. Raise to reject more noise.", apply=HOT),
    Field("general", "vad_soft_endpoint_silence_ms", "int", "Endpointing", 500,
          "Early-commit pause: a complete + speculation-safe command acts "
          "before the full endpoint, cutting latency. 0 disables.", apply=HOT),

    # --- Dashboard ---------------------------------------------------------
    Field("general", "dashboard_port", "int", "Dashboard", 8765,
          "Web dashboard port. Required to reach the settings console."),
    Field("general", "dashboard_host", "str", "Dashboard", "127.0.0.1",
          "Bind address. '127.0.0.1' = local only; '0.0.0.0' exposes it to "
          "your network (set FULLOCH_DASHBOARD_TOKEN in .env first)."),
    Field("general", "dashboard_ssl_certfile", "str", "Dashboard", None,
          "Optional HTTPS cert path (both cert + key required, or TLS is "
          "skipped). In Docker the files must live under ./data."),
    Field("general", "dashboard_ssl_keyfile", "str", "Dashboard", None,
          "Optional HTTPS key path (pairs with dashboard_ssl_certfile)."),

    # --- Home Assistant ----------------------------------------------------
    Field("home_assistant", "url", "str", "Home Assistant", None,
          "Home Assistant base URL, e.g. http://192.168.1.50:8123."),
    Field("home_assistant", "token", "str", "Home Assistant", None,
          "Long-lived access token. Prefer FULLOCH_HA_TOKEN in .env, which "
          "takes priority and keeps the token out of config.", secret=True),
    Field("home_assistant", "timeout", "int", "Home Assistant", 10,
          "HTTP request timeout in seconds."),
    Field("home_assistant", "spotify_entity", "str", "Home Assistant", None,
          "Spotify media_player (friendly name or entity_id). Auto-detected "
          "if blank."),
    Field("home_assistant", "todo_entity", "str", "Home Assistant", None,
          "HA todo list for voice items. Auto-detected if blank."),
    Field("home_assistant", "calendar", "str", "Home Assistant", None,
          "HA calendar Fulloch reads/writes reminders to (e.g. 'Fulloch')."),
    Field("home_assistant", "reminder_poll", "bool", "Home Assistant", True,
          "Poll the reminder calendar every 60s and speak upcoming events. "
          "Disable if a HA automation calls fulloch.speak instead."),

    # --- Notes -------------------------------------------------------------
    Field("notes", "path", "str", "Notes", "./data/notes",
          "Where markdown notes are stored (or point at an Obsidian vault)."),
    Field("notes", "daily_subdir", "str", "Notes", "daily",
          "Subfolder for daily-journal notes (YYYY-MM-DD.md)."),

    # --- Search ------------------------------------------------------------
    Field("search", "searxng_url", "str", "Search", "http://localhost:8080/search",
...
WAKEWORD_PRESETS: tuple = (
    WakewordPreset(
        "hey_atticus", "Hey Atticus", "hey atticus",
    ...
    WakewordPreset(
        "ok_computer", "Ok Computer", "ok computer",
...
def wakeword_presets_as_dicts() -> list:
    return [
        {"id": p.id, "label": p.label, "wakeword": p.wakeword,
         "pattern": p.pattern, "recommended": p.recommended}
...
TIER_PRESETS: tuple = (
    TierPreset(
        ...
        {"asr": {"backend": "qwen-onnx"}, "tts": {"backend": "qwen"},
         "llm": {"backend": "llama"}},
    ),
    TierPreset(
        "cpu_server", "CPU + LLM on another server",
        ...
        {"asr": {"backend": "qwen-onnx"}, "tts": {"backend": "kokoro-onnx"},
         "llm": {"backend": "openai"}},
    ),
    TierPreset(
        "cpu_local", "CPU (fully local)",
...
def tier_presets_as_dicts() -> list:
    return [
        {"id": t.id, "label": t.label, "blurb": t.blurb,
         "models": t.models, "recommended": t.recommended}
