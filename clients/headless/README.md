...
[Click here to see the ESP32-S3 companion project](https://github.com/liampetti/fulloch-satellite)
...
3. For Fulloch's default self-signed HTTPS, set `server.ca_cert` to the dashboard certificate or, preferably, the public `data/certs/fulloch-home-ca.crt` generated by `scripts/create_local_ca.py`. The client keeps certificate and hostname validation enabled.
...
Ctrl+C to stop. Reconnects automatically on connection loss. The server rejects
connections while another satellite is in Conversation mode; the client backs
off exponentially and reconnects once that mode is disabled.
...
| `server.ca_cert` | (none) | CA/leaf certificate used to validate `wss://` server TLS |
...
the playback stream on `tts.end`, `follow_up`, `idle`, or `tts.cancel`. In
conversation mode, the server may additionally send `conversation.transcript`
and `conversation.response`; the client logs both.

With the default half-duplex configuration, `speaking` sends
`{"type":"satellite.mute","muted":true}` and stops mic uplink. `follow_up`,
`idle`, `tts.end`, and `tts.cancel` unmute it. Set `audio.full_duplex: true` only on a
...
- The downlink is `pcm_s16le` at 16 kHz. Tap the same audio that is queued for
  ...
  reference.
- On `tts.end`, drain queued I2S audio, stop the speaker-reference stream, and
  resume normal microphone handling. On `tts.cancel`, stop playback immediately
  and discard queued audio.
